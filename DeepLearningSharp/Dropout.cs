...
namespace DeepLearningSharp
{
    public class Dropout
    {
        ...
        public void train(int epochs, double[][] train_X, int[][] train_Y, bool dropout, double p_dropout, double lr)
        {
            ...
            for (int epoch = 0; epoch < epochs; epoch++)
            {
                ...
                for (int n = 0; n < N; n++)
                {
                    ...
                    dropout_masks = new List<int[]>(n_layers);
                    layer_inputs = new List<double[]>(n_layers + 1);  // +1 for logistic layer
                    ...
                    for (int i = 0; i < n_layers; i++)
                    {
                        ...
                        else layer_input = (double[])layer_output.Clone();

                        layer_inputs.Add((double[])layer_input.Clone());
                        ...
                        if (dropout)
                        {
                            ...
                            dropout_masks.Add((int[])mask.Clone());
                        }
                    ...
                    layer_inputs.Add((double[])layer_output.Clone());
                    ...
                    for (int i = n_layers - 1; i >= 0; i--)
                    {
                        ...
                        if (dropout)
                        {
                            ...
                            {
                                prev_dy[j] *= dropout_masks[i][j];
                            }
                        }

                        dy = new double[hidden_layer_sizes[i]];
                        hiddenLayers[i].backward(layer_inputs[i], dy, layer_inputs[i + 1], prev_dy, prev_W, lr);
                    }
        ...
        public void pretest(double p_dropout)
        {
            for (int i = 0; i < n_layers; i++)
            {
                ...
                if (i == 0) inn = n_in;
                else inn = hidden_layer_sizes[i];

                if (i == n_layers - 1) ou = n_out;
                else ou = hidden_layer_sizes[i + 1];
